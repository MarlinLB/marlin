...
| Provenance is decided: `data-plane/VERSION` is a hand-bumped file, not `git describe`, sourcing `MARLIN_VERSION` for both `marlin.bpf.o` (`.rodata.marlin_version`, `include/marlin/build.h`) and `marlind` via a generated `include/marlin/version.h`. `verifier_stats` versions itself independently and the C# control plane does not version at all yet. Still open: when `0.0.0-dev` becomes a real release number — contingent on packaging, which still does not exist (`docs/DEPLOYMENT.md` §1.2) — and whether the control plane's version is then required to match the datapath's | `data-plane/VERSION`, `data-plane/include/marlin/version.h` | 1 |
