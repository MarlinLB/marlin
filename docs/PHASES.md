...
| Provenance is decided: `marlin.bpf.o` and `marlind` version independently, each a hand-bumped file (`data-plane/bpf/VERSION`, `data-plane/marlind/VERSION`), not `git describe`, each sourcing its own generated header (`MARLIN_BPF_VERSION` via `include/marlin/version.h`, `MARLIND_VERSION` via `include/marlind/version.h`). `marlind` refuses to attach a `marlin.bpf.o` older than a hand-maintained floor (`MARLIND_MIN_BPF_VERSION`, `data-plane/include/marlind/compat.h`). `verifier_stats` versions itself independently and enforces no floor; the C# control plane does not version at all yet. Still open: when `0.0.0-dev` becomes a real release number for each — contingent on packaging, which still does not exist (`docs/DEPLOYMENT.md` §1.2) — whether the two datapath numbers are ever required to move together, and whether the control plane's version is then required to match either | `data-plane/bpf/VERSION`, `data-plane/marlind/VERSION`, `data-plane/include/marlind/compat.h` | 1 |
| Whether `marlind` also carries a *ceiling* on `marlin.bpf.o`, not only the minimum in `data-plane/include/marlind/compat.h`. A floor refuses an object older than marlind expects; nothing bounds the other direction. A renamed `xdp_main` (`marlind/bpf_load.c`), a changed map definition (libbpf reuse failure) and a changed `struct marlin_build` layout (`include/marlind/build.h`'s exact-size match) already fail loudly, so the residue is an object with identical names and definitions but changed semantics — a new `cfg.flags` bit, a reordered `enum marlin_ret`. Those are control-plane surfaces, and the control plane is unversioned. A major-version ceiling is also a no-op for all of `0.x` (SemVer §4 makes every `0.y` bump potentially breaking), so it cannot be evaluated before `1.0.0` — which the provenance row above already makes contingent on packaging | `data-plane/include/marlind/compat.h`, `docs/design/29-versions.md` | 1 |
...
| `balancer.c`'s map-reading helpers (`vip_map`'s `HASH` lookup, `fwd_table`'s and `backends`' `ARRAY` lookups), `marlin_balancer_quic_decode()`'s `bpf_xdp_load_bytes()` call, and `marlin_balancer_validate()`'s `bpf_xdp_get_buff_len()` call have no native-tier stub — `data-plane/tests/stubs/hash_stub.h` is hardcoded to `rl_key`, and no `ARRAY`, `bpf_xdp_load_bytes()` or `bpf_xdp_get_buff_len()` stub exists — so these helpers stay packet-tier-only under `docs/design/24-testing.md`'s three-part test until stubs are written or the gap is accepted as permanent | `docs/design/24-testing.md:157-178`, `data-plane/bpf/balancer.c` | 2b |
| `MARLIN_DROP_ENCAP_LENGTH` has no test at either tier. `marlin_balancer_validate()` compares `bpf_xdp_get_buff_len(ctx)` against `mctx->pkt_len`, which `parser.c:302` derives from `data_end - data`; the two can only disagree for a multi-buffer frame, and `BPF_PROG_TEST_RUN` delivers a single linear buffer, so the packet tier cannot make this branch fire. The native tier is blocked by the missing `bpf_xdp_get_buff_len()` stub in the row above. Either an integration rig on an MTU that forces multi-buffer XDP covers it, or the reason is accepted as unreachable-by-construction and the check is a guard rather than a behaviour | `balancer.c:89-100`, `docs/design/24-testing.md` | 2b |
